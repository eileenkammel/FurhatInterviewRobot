...
class CVAdviceNeed : EnumEntity(speechRecPhrases = true) {
    override fun getEnum(lang: Language): List<String> {
        return listOf(
            "contents:contents, cv contents, contents of a cv, what to put in a cv, how to write a cv",
            "cv with no experience:cv with no experience, how to write a cv if I have no experience, how to write a cv for the first job, first cv",
            "structure:structure, how to structure a cv, what's a good structure for a cv",
            "personal interests:personal interest, what are good personal interest to mention in a cv, should I mention personal interests"
        )
    }
}